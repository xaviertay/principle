...
server <- shinyServer(function(input, output){
  ...
  output$outcome5 <- renderPrint(input$variable3)
  output$plot.3 <- renderPlot({
    if(input$outcomeD=='Diabetes'){
      diab.yes <- subset(diabetes,Outcome=="Diabetes")
      ggplot(diab.yes, aes(x=diab.yes[,input$variable2],y=diab.yes[,input$variable3])) +
        geom_density_2d(aes(fill = ..level..),geom="polygon") +
        xlab(input$variable2) +
        ylab(input$variable3)
    }
    else if(input$outcomeD=="No Diabetes"){
      diab.no <- subset(diabetes,Outcome=="No Diabetes")
      ggplot(diab.no, aes(x=diab.no[,input$variable2],y=diab.no[,input$variable3]))+
        geom_density_2d(aes(fill = ..level..),geom="polygon")+
        xlab(input$variable2) +
        ylab(input$variable3)
    }
  ...
  output$outcome10 <- renderPrint(input$variable10)
  output$plot.4 <- renderPlot({
    diabetes$bmiGroup<-findInterval(diabetes$BMI,c(0,18.5,24.9,29.9,39.9))
    diabetes<-diabetes %>% 
    ...
    levels(diabetes$BMI_Range)<-c("Underweight","Healthy","Overweight","Obese",
                                  "Extremely Obese")
    ggplot(diabetes,aes(x= diabetes[,input$variable10], y=BMI_Range,col=BMI_Range,fill=BMI_Range)) +
      geom_density_ridges() +
      xlab(input$variable10) +
      ylab("BMI Index") +
      theme_ridges() + 
  ...
  output$prediction<-renderPrint({
   ...
   if(percent>=50){
     cat(paste(" You have",percent,"percent of getting diabetes.","\n","Kindly visit your nearest healthcare professional for personalized medical advice."))
   }
   else if(percent<50){
     cat(paste(" You have",percent,"percent of getting diabetes.","\n","But, you are encouraged to do medical checkups to maintain health condition."))
  }
  })
})
              ...
              tabsetPanel(
                navbarMenu("Visualization",icon=icon("fas fa-chart-bar"),
                           ...
                           tabPanel("Ridgeline Chart",
                                    sidebarPanel(selectInput("variable10","Ridgeline Chart",names(diabetes)[1:8],multiple=FALSE,selected="Insulin"),
                                                 submitButton(text="Submit",icon=icon("refresh"),width=NULL)),
                                    mainPanel(h5("Ridgeline Chart of"),verbatimTextOutput("outcome10"),plotOutput("plot.4")))),
                 ...
                 tabPanel("Prediction",icon=icon("fas fa-ambulance"),
                         sidebarPanel(numericInput("Pregnancies","Please key in the value for number of times pregnant",value=0,min=0,max=50),
                                      ...
                                      submitButton(text="Submit",icon=icon("refresh"),width=NULL)),
                         mainPanel(h3("Diabetes Disease Prediction"),h5("The probability of getting diabetes (Percentage):"),verbatimTextOutput("prediction")))
                )
              )
